...
openssl req -new -x509 -key keys/ca/ca_key.pem -out keys/ca/ca_cert.pem -days 365

openssl genrsa -out keys/server/private/server_key.pem 2048

openssl req -new -key keys/server/private/server_key.pem -out keys/server/private/server_csr.pem
...
openssl req -new -key keys/client/client_key.pem -out keys/client/client_csr.pem
